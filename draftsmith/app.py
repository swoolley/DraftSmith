...
class DraftSmithApp:
    ...
    def _build(self) -> None:
        ...
        self.context_body_bytes = QSpinBox()
        self.context_body_bytes.setRange(100, 1_000_000)
        self.context_body_bytes.setValue(self.config.context_body_bytes)
        self.context_body_bytes.setSuffix(" bytes/message")
        scan_form.addRow("Sent body context", self.context_body_bytes)
        supplemental_row, supplemental_layout = QWidget(), QHBoxLayout()
        supplemental_layout.setContentsMargins(0, 0, 0, 0)
        self.supplemental_sent_label = QLineEdit(self.config.supplemental_sent_label)
        self.supplemental_sent_label.setPlaceholderText("Optional Gmail label")
        self.supplemental_sent_label_count = QSpinBox()
        self.supplemental_sent_label_count.setRange(0, 100)
        self.supplemental_sent_label_count.setValue(self.config.supplemental_sent_label_count)
        self.supplemental_sent_label_count.setSuffix(" messages")
        supplemental_layout.addWidget(self.supplemental_sent_label, 1)
        supplemental_layout.addWidget(self.supplemental_sent_label_count)
        supplemental_row.setLayout(supplemental_layout)
        scan_form.addRow("Extra Sent context", supplemental_row)
        self.system_prompt = QTextEdit()
        self.system_prompt.setPlainText(self.config.system_prompt)
        self.system_prompt.setFixedHeight(110)
        scan_form.addRow("System prompt", self.system_prompt)
    ...
    def _prepare_engine(self) -> DraftEngine:
        ...
        self.config.context_body_bytes = self.context_body_bytes.value()
        self.config.supplemental_sent_label = self.supplemental_sent_label.text().strip()
        self.config.supplemental_sent_label_count = self.supplemental_sent_label_count.value()
